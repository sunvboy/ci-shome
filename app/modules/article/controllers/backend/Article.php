		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				$data['listArticle'] = $this->Autoload_Model->_condition(array(
					'module' => 'article',
					'select' => '`object`.`id`, `object`.`title`, `object`.`slug`,`object`.`canonical`, `object`.`catalogueid`, `object`.`catalogue`, `object`.`publish`,`object`.`ishome`, `object`.`highlight`, `object`.`isaside`, `object`.`isfooter`, `object`.`image`, `object`.`created`, `object`.`order`, `object`.`viewed`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM article_catalogue WHERE article_catalogue.id = object.catalogueid) as catalogue_title',
					'where' => ((!empty($extend)) ? '`object`.`userid_created` = '.$this->auth['id'].' AND `alanguage` = \''.$this->fclang.'\'' : '`alanguage` = \''.$this->fclang.'\'' ),
					'keyword' => '(`object`.`title` LIKE \'%'.$keyword.'%\' AND `object`.`description` LIKE \'%'.$keyword.'%\')',
					'catalogueid' => $catalogueid,
					'limit' => $perpage,
				...
				$data['listArticle'] = $this->Autoload_Model->_get_where(array(
					'select' => 'id, catalogueid, catalogue, title, canonical, publish, created, order, viewed, image,  ishome,  highlight,  isaside,  isfooter, (SELECT fullname FROM user WHERE user.id = article.userid_created) as user_created, (SELECT title FROM article_catalogue WHERE article_catalogue.id = article.catalogueid) as catalogue_title',
					'table' => 'article',
					'where_extend' => $extend,
					'where' => array( 'alanguage' => $this->fclang),
