    ...
    public function listCatalogue()
    {
        ...
        if ($config['total_rows'] > 0) {
            ...
            $listCatalogue = $this->Autoload_Model->_get_where(array(
                'select' => 'id, title, publish, created, order, level,ishome,highlight,  isaside,  isfooter,canonical, (SELECT fullname FROM user WHERE user.id = article_catalogue.userid_created) as user_created',
                'table' => 'article_catalogue',
                'where' => array('alanguage' => $this->fclang),
                'keyword' => '(title LIKE \'%' . $keyword . '%\' OR description LIKE \'%' . $keyword . '%\')',
        ...
        $listIS = $this->Autoload_Model->_get_where(array(
            'select' => '*',
            'table' => 'general_is',
            'where' => array('module' => 'article_catalogue', 'publish' => 1),
        ), TRUE);
        if (isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)) {
            foreach ($listCatalogue as $key => $val) {
                ...
                $html = $html . '<td><a class="maintitle" style="' . (($val['level'] == 1) ? 'font-weight:600;' : '') . '" href="' . site_url('articles/backend/articles/view?cataloguesid=' . $val['id'] . '') . '" title="">' . (str_repeat('|----------', (($val['level'] > 0) ? ($val['level'] - 1) : 0)) . $val['title']) . ' ('.$count.')</a>
                        ...
                        $html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="article_catalogue" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
