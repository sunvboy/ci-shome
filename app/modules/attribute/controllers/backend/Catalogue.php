...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title,  keyword,publish, created, order,car,room,tour,level,ishome,image,highlight,isaside,issearch, (SELECT fullname FROM user WHERE user.id = attribute_catalogue.userid_created) as user_created',
				'table' => 'attribute_catalogue',
				'where' => array('alanguage' => $this->fclang),
	...
	public function Update($id = 0){
		...
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title,  description, image,  publish,parentid',
			'table' => 'attribute_catalogue',
