									...
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										<div class="uk-search uk-flex uk-flex-middle mr10">
											<form class="uk-form" id="search">
                                                <div class="hidden">
                                                    <?php echo form_dropdown('view', array(2=>'Khách sạn'), set_value('view',$this->input->get('view')) ,'class="form-control select3 input-sm filter view" '); ?>
                                                </div>
									...
									<?php if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ ?>
										<?php foreach($listCatalogue as $key => $val){ ?>
											<tr class="gradeX" id="cat-<?php echo $val['id']; ?>">
												...
												<td>
                                                    <a class="maintitle"  href="<?php echo site_url('attribute/backend/attribute/view?catalogueid='.$val['id'].''); ?>" ></a><?php echo str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['title']; ?><a href="<?php echo $val['keyword'] ?>"
                                                                                                                                                                                                                                                                             title="Lấy địa chỉ liên kết"
                                                                                                                                                                                                                                                                             onclick="prompt('Lấy địa chỉ liên kết','<?php echo $val['keyword'] ?>'); return false;"><img
                                                                border="0"
                                                                src="template/backend/img/link.png"></a></a>
												...
												<td class="text-center">
													<a type="button" href="<?php echo site_url('attribute/backend/catalogue/update/'.$val['id'].'') ?>" class="btn btn-primary" ><i class="fa fa-edit"></i></a>
