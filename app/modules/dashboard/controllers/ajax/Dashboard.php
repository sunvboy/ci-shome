...
class Dashboard extends MY_Controller {
	...
	public function getLocation(){
		...
		if(isset($locationList) && is_array($locationList) && count($locationList)){
			$i =0 ;foreach($locationList as $key => $val){ $i++;
	...
	public function ajax_delete(){
		...
		if(isset($module[1]) && $module[1] == 'catalogue' && $module[0] == 'tag'){
        ...
        if(isset($post['list']) && is_array($post['list']) && count($post['list'])){
            foreach($post['list'] as $key => $val){
                ...
                if(isset($module[1]) && $module[1] == 'catalogue'){
                    //Xóa bảng Router
                    $deleteRouter = $this->Autoload_Model->_delete(array(
                        'where' => array('param' => $val,'uri' => ''.$module[0].'/frontend/'.$module[1].'/view'),
                        'table' => 'router',
                    ));
                }else if(isset($module[1]) && $module[1] == 'brand'){
                    //Xóa bảng Router
                    $deleteRouter = $this->Autoload_Model->_delete(array(
                        'where' => array('param' => $val,'uri' => ''.$module[0].'/frontend/'.$module[1].'/view'),
                        'table' => 'router',
                    ));
                }else if(isset($module[1]) && $module[1] == 'area'){
                    //Xóa bảng Router
                    $deleteRouter = $this->Autoload_Model->_delete(array(
                        'where' => array('param' => $val,'uri' => ''.$module[0].'/frontend/'.$module[1].'/view'),
                        'table' => 'router',
