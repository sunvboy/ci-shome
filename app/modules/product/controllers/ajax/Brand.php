	...
	public function listCatalogue(){
		...
		if($config['total_rows'] > 0){
			...
			$listCatalogue = $this->Autoload_Model->_get_where(array(
				'select' => 'id,ishome, title, publish, created,image,viewed,canonical, order, (SELECT fullname FROM user WHERE user.id = product_brand.userid_created) as user_created',
				'table' => 'product_brand',
				'keyword' => '(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')',
				'where' => array( 'alanguage' => $this->fclang),
                ...
                $html = $html .'<td>';
					...
					$html = $html .'<td>';
						...
						$html = $html .'<div class="onoffswitch">';
								$html = $html.'<input type="checkbox" '.(($val['ishome'] == 1) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="product_brand" data-title="ishome" data-id="'.$val['id'].'" id="publishishome-'.$val['id'].'">';
								$html = $html .'<label class="onoffswitch-label" for="publishishome-'.$val['id'].'">';
		...
		$html = $html .'<td>';
						$html = $html .'<div class="switch">';
						$html = $html .'<div class="onoffswitch">';
								$html = $html.'<input type="checkbox" '.(($val['publish'] == 0) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="product" data-title="publish" data-id="'.$val['id'].'" id="publish-'.$val['id'].'">';
