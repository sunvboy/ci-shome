...
class Product extends MY_Controller
{
    ...
    public function get_attrid()
    {
        ...
        $detailCatalogue = $this->Autoload_Model->_get_where(array(
            ...
            'where' => array('id' => $catalogueid, 'alanguage' => $this->fclang),
        ));
    ...
    public function create_attribute()
    {
        if ($this->input->post('data')) {
            ...
            $this->form_validation->set_rules('title', 'Tên thuộc tính', 'trim|required|max_length[100]|callback__CheckCanonical');
    ...
    public function create_tags()
    {
        if ($this->input->post('data')) {
            ...
            $this->form_validation->CI =& $this;
            $this->form_validation->set_error_delimiters('', ' / ');
            $this->form_validation->set_rules('title', 'Tiêu đề tag', 'trim|required|max_length[100]|callback__CheckCanonical');
            if ($this->form_validation->run() == FALSE) {
                $errors = validation_errors();
                echo $errors;
                die;
            ...
            if ($this->form_validation->run($this)) {
                ...
                if (isset($insert) && is_array($insert) && count($insert)) {
                    $resultid = $this->Autoload_Model->_create(array('data' => $insert, 'table' => 'tag'));
                    if ($resultid > 0) {
                        if ($resultid > 0) {
                            $canonical = slug($data['title']);
                            if (!empty($canonical)) {
    ...
    public function listproduct()
    {
        ...
        if (!empty($param['tag'])) {
            $json[] = array('tag_relationship as tag2', 'tb1.id = tag2.moduleid AND tag2.module ="product"', 'inner');
            $query = $query . ' AND ( ';
        ...
        if ($config['total_rows'] > 0) {
            ...
            $listproduct = $this->Autoload_Model->_get_where(array(
                'distinct' => 'true',
                'select' => 'tb1.image, tb1.ishome,tb1.highlight,tb1.isaside,tb1.isfooter, tb1.version, tb1.title, tb1.price, tb1.price_sale, tb1.price_contact, tb1.id, tb1.canonical, tb1.viewed, tb1.order, (SELECT account FROM user WHERE tb1.userid_created = user.id) as user_created, tb1.created, tb1.publish, tb1.catalogueid, tb1.quantity_dau_ki, tb1.quantity_cuoi_ki, (SELECT title FROM product_catalogue WHERE product_catalogue.id = tb1.catalogueid) as catalogue_title,tb1.catalogue, (SELECT fullname FROM customer WHERE customer.id = tb1.customerid) as customer_fullname, (SELECT account FROM customer WHERE customer.id = tb1.customerid) as customer_account',
                'table' => 'product as tb1',
        ...
        if (isset($listproduct) && is_array($listproduct) && count($listproduct)) {
            foreach ($listproduct as $key => $val) {
                ...
                $html = $html . '<div class="title"><a class="maintitle" href="' . site_url('product/backend/product/update/' . $val['id']) . '" title="">' . $val['title'] . ' (' . $val['viewed'] . ' lượt xem ) ' . (($val['version'] > 0) ? "(có " . $val['version'] . " phiên bản)" : '') . '</a><a href="' . $href . '" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\'' . $val['canonical'] . '.html\'); return false;"><img border="0" src="template/backend/img/link.png"></a></div>';
                $html = $html . '<div class="catalogue" style="font-size:10px">';
                $html = $html . '<span style="color:#f00000;">Nhóm hiển thị: </span>';
                $html = $html . '<a class="" style="color:#333;" href="' . site_url('article/backend/article/view?catalogueid=' . $val['catalogueid']) . '" title="">' . $val['catalogue_title'] . '</a>' . ((check_array($_catalogue_list)) ? ' ,' : '');
                ...
                if (!empty($val['user_created'])) {
                    ...
                    $html = $html . $val['customer_fullname'];
                    $html = $html . '-';
                    $html = $html . $val['customer_account'];
                }
                        ...
                        $html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="product" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
                ...
                $html = $html . '<a type="button" href="'.site_url('product/backend/product/duplicate/'.$val['id'].'?page=1').'" class="btn btn-info"><i class="fa fa-files-o" aria-hidden="true"></i></a>';
