	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['listBrand'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title,ishome, image, viewed, canonical, publish, created, order, (SELECT fullname FROM user WHERE user.id = product_brand.userid_created) as user_created',
				'table' => 'product_brand',
				'where' => array( 'alanguage' => $this->fclang),
		...
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				if($resultid > 0){
					...
					$this->session->set_flashdata('message-success', 'Thêmthương hiệu sản phẩm mới thành công');
