...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$config['cur_tag_open'] = '<li class="active"><a class="page-numbers active">';
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, image, icon, publish, created, order, level, lft, rgt,ishome,highlight,isaside,isfooter, (SELECT fullname FROM user WHERE user.id = product_catalogue.userid_created) as user_created',
				'table' => 'product_catalogue',
				'where' => array( 'alanguage' => $this->fclang),
