	...
	public function View(){
		...
		if($config['total_rows'] > 0){
			...
			$data['listComment'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, fullname, email, phone, module, detailid, parentid, rate, comment, image, publish,level,comment_reply',
				'table' => 'comment',
				'where' => ($param['module'] != '0')? array('module' => $param['module']):'',
				'keyword' => '(fullname LIKE \'%'.$param['keyword'].'%\' OR email LIKE \'%'.$param['keyword'].'%\' OR phone LIKE \'%'.$param['keyword'].'%\' OR comment LIKE \'%'.$param['keyword'].'%\')',
				'limit' => $config['per_page'],
				'start' => $page * $config['per_page'],
				'order_by' => 'lft asc, id desc',
			), TRUE);
			...
			if(isset($data['listComment']) && is_array($data['listComment']) && count($data['listComment'])){
				...
				foreach($data['listComment'] as $key => $val){
					...
					$listComment .= '<tr>';
						...
						$listComment .= '<td class="text-left">'.str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['fullname'].'</td>';
						...
						$listComment .= '<td class="text-center"><a href="'.$href.'" target="_blank" title="'.$detail['title'].'"><i class="fa fa-link"></i></a><span>(0)</span></td>';
						$listComment .= '<td class="text-left text-comment"><span class="line-1">'.(($val['comment'] != '')? $val['comment']:$val['comment_reply']).'</span></td>';
