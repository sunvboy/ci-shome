...
class Comment extends MY_Controller {
	...
	public function update($id = 0){
		$this->commonbie->permission("comment/backend/comment/update", $this->auth['permission']);
		$id = (int)$id;
		
		//lấy dữ liệu dự vào id
		$detailComment = $this->Autoload_Model->_get_where(array(
			'select' => 'id, fullname, email, phone, module, detailid, rate, comment, image, publish,parentid,comment_reply',
			'table' => 'comment',
			'where' => array('id' => $id),
		));
			...
			if($this->form_validation->run($this)){
				...
				if($result > 0){
					$_insert = array(
						...
						'module' => $detailComment['module'],
						'detailid' => $detailComment['detailid'],
						'parentid' => isset($detailComment['id'])?$detailComment['id']:'',
						'publish' => 1,
						'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
						'alanguage' => $this->fclang,

					);
					$_insert_id = $this->Autoload_Model->_create(array(
						'table' => 'comment',
						'data' => $_insert,
					));
					if($_insert_id > 0){
