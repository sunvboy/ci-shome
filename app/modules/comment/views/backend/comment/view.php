							...
							<div class="mb15">
								<div class="uk-flex uk-flex-middle uk-flex-space-between">
									<div class="wrap-select">
										<?php echo form_dropdown('perpage', $this->configbie->data('perpage'), set_value('perpage'), 'class="form-control filter" id="perpage"');?>
									...
									<tbody id='listComment'>
										<?php if(isset($listComment) && is_array($listComment) && count($listComment)){ ?>
											...
											<tr>
												...
												<td class="text-left"><?php echo str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['fullname']; ?></td>
												<td class="text-center"><?php echo ($val['phone'])? $val['phone']:'';?></td>
												<td class="text-left"><?php echo !empty($val['email'])? $val['email']:'';?></td>
												<td class="text-center"><?php echo !empty($val['parentid'] == 0)?$this->myconstant->get_data($val['module']):'';?></td>
												<td class="text-center"><a href="<?php echo !empty($href)?$href:''?>" target="_blank" title="<?php echo !empty($detail['title'])?$detail['title']:'';?>"><i class="fa fa-link"></i></a><span>(0)</span></td>
												<td class="text-left text-comment"><span class="line-1"><?php echo ($val['comment'] != '')? $val['comment']:$val['comment_reply'];?></span></td>
