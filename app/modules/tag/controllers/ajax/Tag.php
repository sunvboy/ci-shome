	...
	public function listTag(){
		...
		if($config['total_rows'] > 0){
			...
			$listTag = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, publish, created, order, image, (SELECT title FROM tag_catalogue WHERE tag.catalogueid = tag_catalogue.id) as titleCatalogue, (SELECT fullname FROM user WHERE user.id = tag.userid_created) as user_created',
				'table' => 'tag',
				'where' => array('alanguage' => $this->fclang),
				'where_extend' => $extend,
				'keyword' => '(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')',
