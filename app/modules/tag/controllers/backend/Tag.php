...
class Tag extends MY_Controller {
	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['listTag'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, publish, created, order, image, (SELECT title FROM tag_catalogue WHERE tag.catalogueid = tag_catalogue.id) as titleCataalogue, (SELECT fullname FROM user WHERE user.id = tag.userid_created) as user_created',
				'table' => 'tag',
				'where' => array('alanguage' => $this->fclang),
				'where_extend' => $extend,
	...
	public function Update($id = 0){
		...
		$detailTag = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description , image, meta_title, meta_description, publish,catalogueid',
			'table' => 'tag',
