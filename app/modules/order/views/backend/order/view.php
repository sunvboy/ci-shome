                    ...
                    <div class="ibox-title">
                        ...
                        <div class="ibox-tools">
                            ...
                            <ul class="dropdown-menu dropdown-article">
                                <li><a type="button" class="ajax-recycle-all"
                                       data-title="Lưu ý: Khi bạn xóa danh mục bài viết, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện chức năng này!"
                                       data-module="article_catalogue">Xóa tất cả</a>
                                </li>
                            </ul>
                            <a class="close-link">
                    ...
                    <div class="ibox-content" style="position:relative;">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="keyword" name="keyword" value=""
                                           placeholder="Tìm kiếm theo tên, id, mã code, địa chỉ đơn hàng"
                        ...
                        <div class="uk-flex uk-flex-middle uk-flex-space-between">
                            <div class="text-small mb10">Hiển thị từ <?php echo $from; ?> đến <?php echo $to ?> trên
                                tổng số <span id="total_row">
                                    <?php echo $config['total_rows']; ?>
                                </span> bản ghi
                            </div>
                            <div class="text-small text-danger">*Sắp xếp Vị trí hiển thị theo quy tắc: Số lớn hơn được
                        ...
                        $listTotal = $this->Autoload_Model->_get_where(array(
                            'select' => 'order.id,SUM(total_cart_final) as total',
                            'table' => 'order',
                        ));
                        $listTotalCompleted = $this->Autoload_Model->_get_where(array(
                            'select' => 'order.id,SUM(total_cart_final) as total',
                            'table' => 'order',
                            'where' => array('status' =>'completed'),
                        ));
                        $listTotalCompletedAfter = $this->Autoload_Model->_get_where(array(
                            'select' => 'order.id,SUM(total_lai) as total',
                            'table' => 'order',
                            'where' => array('status' =>'completed'),
                        ));
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="show-block-promotion hidden" style="background: #4da9c1">
                                    <div class="inner">
                                        <div class="title">
                                            <div>Tổng tiền đơn hàng: <span id="totalALL"><?php echo !empty($listTotal)?addCommas($listTotal['total']):0?></span> VNĐ</div>
                                            <div>Tổng tiền đơn hàng "ĐÃ HOÀN THÀNH": <span id="totalCompleted"><?php echo !empty($listTotalCompleted)?addCommas($listTotalCompleted['total']):0?></span> VNĐ</div>
                                            <div>Tổng tiền đơn hàng "LÃI" SAU "ĐÃ HOÀN THÀNH": <span id="totalCompletedAfter"><?php echo !empty($listTotalCompletedAfter)?addCommas($listTotalCompletedAfter['total']):0?></span> VNĐ</div>
                                        </div>
                                ...
                                <table class="footable table table-stripped toggle-arrow-tiny" data-page-size="15">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-right hidden">Tổng tiền lãi</th>
                                    ...
                                    <?php if (isset($listorder) && is_array($listorder) && count($listorder)) { ?>
                                        <?php foreach ($listorder as $key => $val) { ?>
                                            <tr>
                                                ...
                                                <td class="text-right hidden"><?php echo !empty(addCommas($val['status'] == 'completed')) ? addCommas($val['total_lai']) : 0 ?></td>
                                                ...
                                                <td class="text-center">
                                                    <a type="button"
                                                       href="<?php echo site_url('order/backend/order/update/' . $val['id'] . '') ?> "
                                                       class="not-decoration js_open_windown"><span
                                                                class="label  label-info">Cập nhật</span></a>
                                                    <a type="button" class="not-decoration   ajax-delete-order"
                                                       data-id="<?php echo $val['id'] ?>" data-module="order"><span
                                                                class="label label-warning">Xóa</span></a>
                                                </td>
                                            </tr>
                                        <?php }
                                    } ?>
