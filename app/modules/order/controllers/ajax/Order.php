...
class Order extends MY_Controller {
    ...
    public function ajax_delete(){
        $param['id'] = $id = (int)$this->input->post('id');
        $id = (int)$id;
        ...
        //echo "<pre>";var_dump($data['data_order']['list_product']);die;
        if(!empty($data['data_order']['list_product'])){
            foreach ($data['data_order']['list_product'] as $key=>$val){
	...
	public function listorder(){
        ...
        if(isset($param['promotionalid']) && !empty($param['promotionalid'])){
            $query = $query.' AND userid_created = '.$this->auth['id'].' ';
        }
        if(isset($param['date_added']) && !empty($param['date_added']) && empty($param['date_modified'])){
            $query = $query.' AND created >= \'' . $param['date_added'].' 00:00:00' . '\' AND created <= \'' . $param['date_added'].' 23:59:59' . '\'';
        }
        if(isset($param['date_modified']) && !empty($param['date_modified']) && empty($param['date_added'])){
            $query = $query.' AND created <= \'' . $param['date_modified'].' 23:59:59' . '\'';
        }
        if(isset($param['date_added']) && !empty($param['date_added']) && isset($param['date_modified']) && !empty($param['date_modified'])){
            $query = $query.' AND created >= \'' . $param['date_added'].' 00:00:00' . '\' AND created <= \'' . $param['date_modified'].' 23:59:59' . '\'';
        }

        /*if(isset($param['date_added']) && !empty($param['date_added'])){
            $query = $query.' AND created = '.$param['date_added'].' ';
        }if(isset($param['date_modified']) && !empty($param['date_modified'])){
            $query = $query.' AND updated = '.$param['date_modified'].' ';
        }*/

        if(isset($param['status']) && $param['status'] != '-1' && $param['status'] != ''){
		...
		if($config['total_rows'] > 0){
			...
			$listorder = $this->Autoload_Model->_get_where(array(
				'select' => 'id, fullname, created, updated, order, total_cart_final, status, phone,total_lai',
				'table' => 'order',
				'query' => $query,
				'keyword' => '(code LIKE \'%'.$keyword.'%\' OR id LIKE \'%'.$keyword.'%\' OR fullname LIKE \'%'.$keyword.'%\' OR phone LIKE \'%'.$keyword.'%\' OR email LIKE \'%'.$keyword.'%\' OR address_detail LIKE \'%'.$keyword.'%\')',
		 ...
		 if(isset($listorder) && is_array($listorder) && count($listorder)){ 
			foreach($listorder as $key => $val){
				...
				$html = $html.'<tr>';
		            ...
		            $html = $html.'<td class="text-right hidden">'.addCommas($total_lai).'</td>';
		...
		echo json_encode(array(
			...
			'listTotal' => !empty($listTotal)?addCommas($listTotal['total']):0,
			'listTotalCompleted' => !empty($listTotalCompleted)?addCommas($listTotalCompleted['total']):0,
			'listTotalCompletedAfter' => !empty($listTotalCompletedAfter)?addCommas($listTotalCompletedAfter['total']):0,
		));die();
	}
}
