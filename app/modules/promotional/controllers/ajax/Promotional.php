...
class Promotional extends MY_Controller {
	...
	public function get_moduleid(){
		...
		if($module == 'product'){
			$object = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, price,price_sale, quantity_cuoi_ki, code, image',
				'table' => $module,
				'where_in' => $id,
				'where_in_field' =>'id',
			),true);
			...
			if(isset($object) && is_array($object) && count($object)){
				foreach($object as $key => $val){
					...
					if(isset($val['price_sale']) && $val['price_sale'] != '' && $val['price_sale'] > 0){
						$price_final = $val['price_sale'];
					}
		    			...
		    			$html = $html . '<div class="uk-flex uk-flex-middle uk-flex-space-between">';
		    				$html = $html . '<div class="uk-flex uk-flex-middle">';
			        			...
			        			$html = $html . '<div>';
									...
									$html = $html . '<div class="content">Giá bán :<b style="    text-decoration: line-through;">'.addCommas($price_final).'</b> còn <b class="text-danger">'.addCommas($price_final*(100 - $discount_value)/100).'</b> <sup> đ</sup>/ <span class="m-r-xs">Tồn cuối: <b> '.$val['quantity_cuoi_ki'].'</b></span></div>';
								$html = $html . '</div>';
		    				$html = $html . '</div>';
			...
			$object = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, price,price_sale, image',
				'table' => $module,
				'where_in' => $id,
				'where_in_field' =>'id',
			),true);
					...
					$html = $html . '<div class="p-xxs choose-moduleid m-b-sm" data-id='.$val['id'].'>';
		    			$html = $html . '<div class="uk-flex uk-flex-middle uk-flex-space-between">';
		    				$html = $html . '<div class="uk-flex uk-flex-middle">';
			        			...
			        			$html = $html . '<div>';
			        				$html = $html . '<div class="title">'.$val['title'].'</div>';
									$html = $html . '<div class="content">Giá bán :<b> '.$price_final.'</b><sup> đ</sup></b></span></div>';
								$html = $html . '</div>';
		    				$html = $html . '</div>';
		    				$html = $html . '<div class="del m-r-sm" data-id="'.$val['id'].'"><i class="fa fa-times" aria-hidden="true"></i></div>';
		    			$html = $html . '</div>';
