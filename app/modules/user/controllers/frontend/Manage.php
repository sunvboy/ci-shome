...
//            $album=$this->input->post('album');
            if ($this->form_validation->run($this)) {
                ...
                if ($flag > 0) {
                    //var_dump($this->upload->do_upload('profile_pic'));die;
                    //upload images
    ...
    public function history($page = 1)
    {
        ...
        $data['canonical'] = base_url('history');
        $data['meta_title'] = "Lịch sử";
        $data['meta_description'] = "Lịch sử";
        $data['og_type'] = 'order';
        $data['template'] = 'user/frontend/manage/history';
        $this->load->view('homepage/frontend/layout/home', isset($data) ? $data : NULL);
    }
    ...
    public function order_information()
    {
        ...
        $detailorder = $this->Autoload_Model->_get_where(array(
            'select' => 'id,code, fullname, created, cart_json, updated, order, total_cart_final, status,	quantity, promotional_detail, fullname , phone, email, note, address_detail, cityid, , cart_json, extend_json, districtid, wardid,
