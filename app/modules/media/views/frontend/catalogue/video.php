					...
					<?php if (isset($mediaList) && is_array($mediaList) && count($mediaList)) { ?>
						<?php foreach ($mediaList as $key => $val) {
							...
							<div class="col-md-3 col-sm-3 col-xs-3">
								<div class="item1">
									<div class="image">
										<a href="<?php echo !empty($detailCatalogue['isaside']==1)?$val['video_iframe']:$href ?>"  <?php if($detailCatalogue['isaside']==1){?>target="_blank" <?php }?>><img src="<?php echo $val['image'] ?>"
																		   alt="<?php echo $val['title'] ?>"></a>
									</div>
									<div class="nav-image">
										<h3 class="title"><a href="<?php echo !empty($detailCatalogue['isaside']==1)?$val['video_iframe']:$href ?>" <?php if($detailCatalogue['isaside']==1){?>target="_blank" <?php }?>><?php echo $val['title'] ?></a>
										</h3>
										<ul>
											<li>
