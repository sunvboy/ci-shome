...
class Media extends MY_Controller {
	...
	public function view($id = 0){
		...
		$data['relatedmedia'] = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, image,viewed',
			'table' => 'media',
			'where' => array('id!= ' => $detailMedia['id'], 'catalogueid' => $detailCatalogue['id'],'alanguage' => $this->fc_lang),
		...
		if(svl_ismobile()){
			if($detailCatalogue['layoutid'] == 2){
				$data['template'] = 'media/mobile/media/gallery';
			}else{
				$data['template'] = 'media/mobile/media/video';
			}
