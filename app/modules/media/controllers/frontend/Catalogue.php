...
class Catalogue extends MY_Controller {
	...
	public function View($id = 0, $page = 1){
		...
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, canonical, image, lft, rgt, meta_keyword, meta_title, meta_description, description, layoutid,isaside',
		...
		if($config['total_rows'] > 0){
			...
			$config['cur_tag_open'] = '<li class="page-numbers active"><a >';
		...
		if(svl_ismobile()){
			if($detailCatalogue['layoutid'] == 2){
				$data['template'] = 'media/mobile/catalogue/gallery';
			}else{
				$data['template'] = 'media/mobile/catalogue/video';
			}
