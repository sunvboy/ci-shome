	...
	public function listSupport(){
		...
		if($config['total_rows'] > 0){
			...
			$listSupport = $this->Autoload_Model->_get_where(array(
				'select' => 'id, catalogueid, fullname, email, phone, zalo, skype, image, publish,address, (SELECT title FROM support_catalogue WHERE support_catalogue.id = support.catalogueid) as catalogueTitle',
				'table' => 'support',
			...
			if(isset($listSupport) && is_array($listSupport) && count($listSupport)){
				foreach($listSupport as $key => $val){
					...
					$html = $html .'<a href="'.site_url('support/backend/support/update/'.$val['id'].'').'" title="" class="image img-cover hidden">';
					...
					$html = $html .'<p><strong class="text-blue">Địa chỉ:</strong> '.$val['address'].'</p>';
