$(document).ready(function(){
	...
	$(document).on('submit','#form_update_attr',function(e){
		...
		let ajaxUrl = 'tour/ajax/tour/update_attr';
		$.post(ajaxUrl, {
			param: param},
			function(data){
				if(data=='true'){
	...
	$(document).on('submit','#form_update_catalogue',function(e){
		...
		let ajaxUrl = 'tour/ajax/tour/update_catalogue';
		$.post(ajaxUrl, {
	...
	//thêm highlight
    $(document).on('click','.add-attr-highlight',function(){
        let _this = $(this);
        render_attr_highlight();
    })
    function render_attr_highlight(){
        let html ='';
        html = html + '<div class="form-group desc-more-highlight">'
        html = html + '<div class="col-lg-12" >'
        html = html + '<textarea name="content_highlight[]" class="form-control" placeholder="Mô tả"></textarea>'
        html = html + '<button class="delete-attr-highlight" type="button">X</button></div>'
        html = html + '</div>'
        html = html + '</div>'
        $('.attr-more-highlight').append(html);
        //text box increment
    }
    $(document).on('click','.delete-attr-highlight',function(){
        let _this = $(this);
        _this.parents('.desc-more-highlight').remove();
    });
	//end

	//thêm info
	$(document).on('click','.add-attr-info',function(){
		let _this = $(this);
		render_attr_info();
	})
	function render_attr_info(){
		let html ='';
		html = html + '<div class="form-group desc-more-info">'
		html = html + '<div class="col-lg-12" >'
		html = html + ' <input type="text" name="content_info[title][]" value="" class="form-control" placeholder="Tiêu đề">'
		html = html + '<textarea name="content_info[description][]" class="form-control" placeholder="Mô tả"></textarea>'
		html = html + '<button class="delete-attr-info" type="button">X</button></div>'
		html = html + '</div>'
		$('.attr-more-info').append(html);
		//text box increment
	}
	$(document).on('click','.delete-attr-info',function(){
		let _this = $(this);
		_this.parents('.desc-more-info').remove();
	});
	//end
	...
	$(document).on('click','.ajax_delete_tour_all',function(){
		let _this = $(this);
		...
		swal({
			...
			function (isConfirm) {
				if (isConfirm) {
					let ajax_url = 'tour/ajax/tour/ajax_delete_tour_all';
						$.post(ajax_url, {
							post: param},
							function(data){
								if(data == 0){
	...
	// xóa sp
	$(document).on('click','.ajax_delete_tour',function(){
		let _this = $(this);
		let param = {
			'title' : _this.attr('data-title'),
		...
		swal({
			...
			function (isConfirm) {
				if (isConfirm) {
					let ajax_url = 'tour/ajax/tour/ajax_delete_tour';
						$.post(ajax_url, {
	...
	if($('#tour_catalogue').length){
		select2($('#tour_catalogue'));
	}
	if(typeof catalogueid !='undefined'  ){
		pre_select2('tour_catalogue',catalogueid);
	}
	if($('#tag').length){
		select2($('#tag'));
	}
	...
	$(document).on('click' ,'#update_price_tour' ,function(){
		let data = [];
		let title = '';
		...
		function (isConfirm) {
			if (isConfirm) {
				let ajaxUrl = 'tour/ajax/tour/update_price_tour';
				$.post(ajaxUrl, {
	...
	$(document).on('select2:select','select[name="catalogueid"]', function(e){
		...
		let ajax_url = 'tour/ajax/tour/get_attrid';
		$.post(ajax_url, {
	...
	$(document).on('submit','#form_create_attribute',function(e){
		...
		let ajaxUrl = 'tour/ajax/tour/create_attribute';
		$.post(ajaxUrl, {
	...
	//thêm mới tag
	$(document).on('submit','#update-tag',function(e){
		e.stopPropagation();
		let title = $('#update-tag input[name="title"]').val();
		let data = $(this).serializeArray();
		let ajaxUrl = 'tour/ajax/tour/create_tags';
		$.post(ajaxUrl, {
			data: data, title:title},
			function(data){
				if(data=='true'){
					swal("Thêm mới thành công!", "Thêm mới tag thành công.", "success");
					$('.modal-footer .btn-white').trigger("click");
					$("#update-tag .error").addClass('hidden');
				}else{
					$("#update-tag .error").removeClass('hidden');
                	$("#update-tag .error .alert").html(data);
				}
			});
		return false;
	});
	...
	$(document).on('change','.publish',function(){
		...
		let formURL = 'tour/ajax/tour/status';
			$.post(formURL, {
				objectid: objectid},
				function(data){
...
function get_list_object(page = 1){
	...
	console.log(param)
	let ajaxUrl = 'tour/ajax/tour/listtour';
	$.get(ajaxUrl, {
