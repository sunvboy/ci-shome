$(document).ready(function () {
    ...
    $(document).on('submit', '#update-tag', function (e) {
        ...
        let data = $(this).serializeArray();
        let ajaxUrl = 'product/ajax/product/create_tags';
        $.post(ajaxUrl, {
                data: data, title: title
            },
            function (data) {
...
function render_version(title = '', price = '', code = '', attribute1 = '', attribute2 = '') {
    ...
    html = html + '<div class="uk-flex uk-flex-middle hidden">';
    ...
    html = html + '<td class="hidden"><input type="text" name="barcode_version[]" value="" class="form-control"  autocomplete="off" ></td>';
    html = html + '</tr>';
    return html;
}
...
$('.onlyCheckbox').change(function(e){
	var str = '';
	$('.onlyCheckbox').each(function(){
		if($(this).val() != 0  && $(this).prop('checked') == true){
            str = str + '<tr>';
            str = str + '<td>';
			str = str + '<input type="text" value="'+$(this).attr('data-title')+'" name="attrTITLE[]" class="form-control" readonly>';
            str = str + '</td>';
            str = str + '<td>';
            str = str + '<input type="hidden" name="attrID[]" value="'+$(this).val()+'" class="form-control">';
            str = str + '<input type="text" name="quantity[]" value="'+$(this).attr('data-quantity')+'" class="form-control" autocomplete="off">';
            str = str + '</td>';
            str = str + '</tr>';
		}
	});
