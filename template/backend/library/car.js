$(document).ready(function(){
	...
	$(document).on('submit','#form_update_attr',function(e){
		...
		let ajaxUrl = 'car/ajax/car/update_attr';
		$.post(ajaxUrl, {
			param: param},
			function(data){
				if(data=='true'){
	...
	$(document).on('submit','#form_update_catalogue',function(e){
		...
		let ajaxUrl = 'car/ajax/car/update_catalogue';
		$.post(ajaxUrl, {
	...
	$(document).on('click','.ajax_delete_car_all',function(){
		let _this = $(this);
		...
		swal({
			...
			function (isConfirm) {
				if (isConfirm) {
					let ajax_url = 'car/ajax/car/ajax_delete_car_all';
						$.post(ajax_url, {
							post: param},
							function(data){
								if(data == 0){
	...
	// xóa sp
	$(document).on('click','.ajax_delete_car',function(){
		let _this = $(this);
		let param = {
			'title' : _this.attr('data-title'),
		...
		swal({
			...
			function (isConfirm) {
				if (isConfirm) {
					let ajax_url = 'car/ajax/car/ajax_delete_car';
						$.post(ajax_url, {
	...
	if($('#car_catalogue').length){
		select2($('#car_catalogue'));
	}
	if(typeof catalogueid !='undefined'  ){
		pre_select2('car_catalogue',catalogueid);
	}
	if($('#tag').length){
		select2($('#tag'));
	}
	...
	$(document).on('click' ,'#update_price_car' ,function(){
		let data = [];
		let title = '';
		...
		function (isConfirm) {
			if (isConfirm) {
				let ajaxUrl = 'car/ajax/car/update_price_car';
				$.post(ajaxUrl, {
	...
	$(document).on('select2:select','select[name="catalogueid"]', function(e){
		...
		let ajax_url = 'car/ajax/car/get_attrid';
		$.post(ajax_url, {
	...
	$(document).on('submit','#form_create_attribute',function(e){
		...
		let ajaxUrl = 'car/ajax/car/create_attribute';
		$.post(ajaxUrl, {
	...
	$(document).on('submit','#update-tag',function(e){
		...
		let data = $(this).serializeArray();
		let ajaxUrl = 'car/ajax/car/create_tags';
		$.post(ajaxUrl, {
			data: data, title:title},
			function(data){
				if(data=='true'){
	...
	$(document).on('change','.publish',function(){
		...
		let formURL = 'car/ajax/car/status';
			$.post(formURL, {
				objectid: objectid},
				function(data){
...
function get_list_object(page = 1){
	...
	console.log(param)
	let ajaxUrl = 'car/ajax/car/listcar';
	$.get(ajaxUrl, {
