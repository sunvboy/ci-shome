$(document).ready(function(){
	if($('#software_catalogue').length){
		select2($('#software_catalogue'));
	}
	if(typeof catalogueid !='undefined'  ){
		pre_select2('software_catalogue',catalogueid);
	}
	...
	$(document).on('change','.publish',function(){
		...
		let formURL = 'software/ajax/software/status';
			$.post(formURL, {
				objectid: objectid},
				function(data){
...
function get_list_object(param){
	let ajaxUrl = 'software/ajax/software/listsoftware';
	$.get(ajaxUrl, {
